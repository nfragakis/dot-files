...
Panel {
  ...
  // Provider-neutral data written by the bundled sync service. QML never sees
  ...
  function applyDashboard(raw) {
    ...
    if (raw) {
      try {
        ...
        if (parsed && parsed.version === 1) document = parsed
      } catch (error) {
        document = null
      }
    }
  ...
  function openDashboardUrl(value) {
    var url = Model.safeOpenUrl(value)
    if (!url) return
  ...
  FileView {
    id: dashboardFile
    path: (Quickshell.env("HOME") || "") + "/.local/state/omarchy/calendar-dashboard.json"
    watchChanges: true
    printErrors: false
    onLoaded: root.applyDashboard(text())
    onLoadFailed: root.applyDashboard("")
  ...
  KeyboardPanel {
    ...
    PanelKeyCatcher {
      ...
      onMoveRequested: function(dx, dy) {
        if (root.dayViewOpen) return
        if (dx !== 0) root.moveMonth(dx)
        if (dy !== 0) root.moveYear(dy)
      }
      onActivateRequested: {
        if (root.dayViewOpen) root.closeDay()
        else root.goToToday()
      }
      onCloseRequested: {
        if (root.dayViewOpen) root.closeDay()
      ...
      Flickable {
        ...
        interactive: !root.dayViewOpen && (contentHeight > height || contentWidth > width)
        ...
        Column {
          ...
          Item {
            visible: !root.dayViewOpen
            width: parent.width
            height: visible ? heroRow.height : 0
          ...
          Item {
            visible: !root.dayViewOpen && root.birthYear > 0
            width: parent.width
                ...
                Row {
                  ...
                  Repeater {
                    ...
                    Rectangle {
                      ...
                      TapHandler {
                        onTapped: root.openDay(dayCell.modelData.key)
                      }
                    }
                  }
                }
          ...
          DayView {
            ...
            tasks: root.dashboardTasks
            foreground: root.contentForeground
            fontFamily: root.contentFontFamily
            statusText: root.dashboardStatus
            onBackRequested: root.closeDay()
            onOpenUrlRequested: function(url) { root.openDashboardUrl(url) }
          }
        }
      }
    }
