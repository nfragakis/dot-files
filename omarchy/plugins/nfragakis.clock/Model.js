...
function monthGrid(year, month, weekStart, todayKey, eventIndex) {
  ...
  for (var w = 0; w < 6; w++) {
    ...
    for (var d = 0; d < 7; d++) {
      ...
      var dots = eventIndex ? eventColors(eventIndex, key, 3) : []
      days.push({
        key: key,
        year: cellYear,
        month: cellMonth,
...
// ---- Calendar dashboard data. The provider sync owns network and account
// details; the QML only indexes the normalized document it writes.
...
function eventsForDateKey(index, dateKeyValue) {
  if (!index || !dateKeyValue) return []
...
function dateFromKey(value, fallback) {
  ...
  if (!isFinite(year) || !isFinite(month) || !isFinite(day)) return fallback
...
function safeOpenUrl(value) {
  ...
  if (text.indexOf("https://") === 0) return text
  // Evolution owns this local scheme. It is constructed by the trusted sync
  // adapter and opened directly through Qt, never interpolated into a shell.
  if (text.indexOf("calendar:///?") === 0) return text
  return ""
}

function dashboardStatus(document, nowMs) {
  if (!document || !document.syncedAt) return "No dashboard sync yet"
  var synced = Date.parse(document.syncedAt)
  if (isNaN(synced)) return "Dashboard sync time is invalid"
  var minutes = Math.max(0, Math.floor((nowMs - synced) / 60000))
  var errors = document.errors || []
  if (errors.length > 0) return errors.length + " source error" + (errors.length === 1 ? "" : "s")
  if (minutes > 20) return "Last synced " + minutes + "m ago"
  return "Synced " + (minutes < 1 ? "now" : minutes + "m ago")
}
