...
CODEX_SUCCESS = """session='66666666-7777-8888-9999-aaaaaaaaaaaa' if 'fork' in sys.argv else '11111111-2222-3333-4444-555555555555'
emit({'type':'thread.started','thread_id':session})
emit({'type':'turn.started'})
emit({'type':'item.completed','item':{'id':'answer','type':'agent_message','text':'Hello from Codex'}})
...
class Bridge(unittest.TestCase):
    ...
    def agent(self,body):
        self.tool('claude',CLAUDE_PRELUDE+body)

    def codex(self,body=CODEX_SUCCESS):
    ...
    def test_codex_background_result_privacy_and_fork(self):
        self.tool('omarchy-default-agent', 'print("codex")')
        self.codex()
        ident = self.new()
        shown = self.wait(ident)
        self.assertEqual(shown['job']['provider'], 'codex')
        self.assertEqual(shown['job']['sessionId'], SESSION)
        self.assertEqual(shown['output'], 'Hello from Codex')
        self.assertTrue(shown['job']['canContinue'])
        folder = self.store / ident
        argv = json.loads((folder/'argv.json').read_text())
        command = argv.index('exec')
        self.assertEqual(argv[command:command + 2], ['exec', '--json'])
        ...
        result = self.wait(child)
        self.assertEqual(result['job']['provider'], 'codex')
        self.assertEqual(result['job']['sessionId'], CODEX_SESSION)
        self.assertEqual(result['transcript'][:2], shown['transcript'])
        argv = json.loads((self.store/child/'argv.json').read_text())
        command = argv.index('exec')
        self.assertEqual(argv[command:command + 2], ['exec', 'fork'])
        self.assertEqual(argv[-3:-1], [SESSION, '-'])
        self.assertEqual((self.store/child/'stdin.txt').read_text(), 'Shorter')

    def test_codex_stream_keeps_only_public_status_and_answer(self):
        self.tool('omarchy-default-agent', 'print("codex")')
        self.codex("""emit({'type':'thread.started','thread_id':'11111111-2222-3333-4444-555555555555'})
emit({'type':'turn.started'})
emit({'type':'item.started','item':{'id':'tool','type':'command_execution','command':'SECRET-ARG'}})
emit({'type':'item.completed','item':{'id':'tool','type':'command_execution','aggregated_output':'SECRET-TOOL-OUTPUT'}})
emit({'type':'item.completed','item':{'id':'reason','type':'reasoning','text':'HIDDEN-REASONING'}})
emit({'type':'item.completed','item':{'id':'answer','type':'agent_message','text':'Safe answer'}})
emit({'type':'turn.completed','usage':{'input_tokens':1,'output_tokens':2}})
""")
        shown = self.wait(self.new())
        self.assertEqual(shown['output'], 'Safe answer')
        self.assertIn('Running a command', json.dumps(shown['transcript']))
        serialized = json.dumps(shown)
        for secret in ('HIDDEN-REASONING', 'SECRET-ARG', 'SECRET-TOOL-OUTPUT'):
...
""")
    ...
    def test_unsupported_provider(self):
        for selected in ('', 'other'):
            ...
            self.assertIn('Codex and Claude',error)
