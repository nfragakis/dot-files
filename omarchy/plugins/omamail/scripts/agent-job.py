...
class CodexStream:
    ...
    def answer(self, text):
        valid_text(text)
        self.value['transcript'].append({'role': 'assistant', 'text': text})
        self.value['output'] = text
    ...
    def event(self, value):
        ...
        if kind == 'thread.started':
            ...
            if kind == 'item.started':
                ...
                if item_kind in CODEX_TOOL_LABELS:
                    self.status(CODEX_TOOL_LABELS[item_kind])
            elif item_kind == 'agent_message':
                self.answer(item.get('text', ''))
            elif item_kind in CODEX_TOOL_LABELS and item_id not in self.started:
                self.status(CODEX_TOOL_LABELS[item_kind])
...
def read_job(fd, ident):
    ...
    if job.get('provider', 'claude') not in PROVIDERS:
...
def refresh(fd, ident):
    ...
    if error:
        job['error'] = error
    job['canContinue'] = job['resultReady'] and job.get('provider', 'claude') in PROVIDERS and bool(job.get('sessionId'))
    return job, output
...
def default_provider():
    ...
    if probe.returncode:
        raise ValueError('Could not read the system AI preference.')
    try:
        provider = selected.decode('utf-8', errors='strict').strip()
    except UnicodeError:
        raise ValueError('The system AI preference is invalid.')
    if provider not in PROVIDERS:
        raise ValueError('Background AI supports Codex and Claude. Choose one in the system AI settings, then retry.')
    if not shutil.which(provider):
        raise ValueError(provider.title() + ' is not installed. Install and sign in to the system AI, then retry.')
    return provider


def provider_stream(provider, history):
    if provider == 'claude':
        return ClaudeStream(history)
    if provider == 'codex':
        return CodexStream(history)
    raise ValueError('Unsupported saved AI provider')


def provider_command(provider, resume):
    if provider == 'claude':
        command = ['claude', '-p', '--verbose', '--output-format', 'stream-json',
                   '--include-partial-messages', '--permission-mode', 'dontAsk']
        if resume:
            command += ['--resume', resume, '--fork-session']
        return command
    if provider == 'codex':
        common = ['--json', '--skip-git-repo-check',
                  '-c', 'approval_policy="never"', '-c', 'sandbox_mode="read-only"']
        return (['codex', 'exec', 'fork'] + common + [resume, '-'] if resume
...
def new(base, path):
    ...
    if context.get('parent'):
        ...
        context = previous
        provider = parent.get('provider', 'claude')  # Native history belongs to its original provider.
    else:
...
def run(base, path, ident):
    ...
    with directory(base, ident) as fd:
        with locked(base):
            ...
            write(fd, 'job.json', job)
        parser = provider_stream(job.get('provider', 'claude'), display(fd)['transcript'])
        ...
        command = provider_command(job.get('provider', 'claude'), job.get('resume', ''))
        child = None
        failure = ''
