...
page or separate Agent page. The background adapter supports Codex and Claude;
other defaults produce an inline explanation without opening a terminal or picker.
The selected CLI uses its normal system login, model and provider configuration.
...
The worker runs silently in the background. Public status events appear while
tools run. Claude sends partial text; Codex publishes completed agent messages.
Raw tool arguments/results, diagnostics, and internal reasoning are not displayed.
Completed requests can be followed up in the same native provider conversation.
...
`scripts/agent-job.py` reads requests on stdin and launches the selected Codex or
Claude CLI with non-interactive JSON event output. Mail and questions reach the
CLI through stdin, never process arguments. No terminal launcher is invoked.
...
Codex runs with approval prompts disabled and its read-only sandbox selected, so
the background worker cannot pause for input or edit local files. Claude runs
with non-interactive `dontAsk` permissions. Permission or login failures appear
in the panel. Existing system model, provider and authentication configuration
still applies. Treating mail as untrusted context remains an AI instruction for
any access that the selected CLI permits. Supplied content goes to the provider
configured for the system AI.
...
Backend tests use synthetic Codex and Claude streams and inspect actual process arguments,
...
A real installed-Codex smoke check also passed two synthetic turns: the second
