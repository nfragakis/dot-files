...
Item {
  ...
  // Open on the cursor, like every other acting key.
  function openLabelPicker() {
    if (!service || (cursorId === "" && !selectionActive)) return false
    // A merged list has no single account whose labels can be destinations.
    if (service.unified) {
  ...
  // A ticked row acts on the selection; an unticked row acts alone.
  ...
  // `onlyCursor` is the row menu opened outside the current selection.
  ...
  function runShortcut(id, sequence) {
    ...
    // In Drafts, opening a draft from the keyboard edits it. In every other
    // mailbox this opens the selected message.
